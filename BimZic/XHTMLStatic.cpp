...
#ifdef _GREG_
void CXHTMLStatic::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	HDC hDC;
...
//=============================================================================
{
	...
	// >>> TEST <<<
	//int iid;
	//iid = this->GetDlgCtrlID();
	//theApp.Log.Write(_T("> HTMLText ID : %d"), iid);
	// >>> TEST <<<
	...
	// Create a temporary work buffer where all graphicals operations will be done
	Bitmap WorkBuffer(rect.Width(), rect.Height());
	Graphics wb(&WorkBuffer);
	// Compute the position and size of the client area in the parent area copie le fond couvert par le bouton depuis le background dans le bitmap
	CRect ScrRect;
	GetWindowRect(ScrRect);
	GetParent()->ScreenToClient(ScrRect);
	// wbrect is the size of the area
	RectF wbrect(0, 0, (Gdiplus::REAL)nRectWidth, (Gdiplus::REAL)nRectHeight);
	// Compute X position depending on RTL flag

	REAL Left = 0;
	if (!theApp.m_bIsRTL)
		Left = (Gdiplus::REAL)ScrRect.left;
	else
		Left = (Gdiplus::REAL)(((CBimZicDlg *)(theApp.m_pMainWnd))->m_pDoubleBuffer->GetWidth() - (ScrRect.left + nRectWidth));
	// Copy background from parent (parent is doublebuffer)
	wb.DrawImage(((CBimZicDlg *)(theApp.m_pMainWnd))->m_pDoubleBuffer, wbrect, Left/*(Gdiplus::REAL)ScrRect.left*/, (Gdiplus::REAL)ScrRect.top, (Gdiplus::REAL)nRectWidth, (Gdiplus::REAL)nRectHeight, UnitPixel);
...
//		Bitmap WorkBuffer(nRectWidth, nRectHeight);
		WorkBuffer.GetHBITMAP(Color::White, &m_hBitmap);
...
//=============================================================================
{
	return FALSE;//TRUE;//CStatic::OnEraseBkgnd(pDC);
}
